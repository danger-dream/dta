...
* 飞桨OCR部署完成后的地址是http://host:port/predict/ocr_system，我自己代理了一层到cdn，要用的话记得改成你自己的url
* openl我默认禁用了，免费额度太少，应急用吧，反正很多是重合的，不重合的效果也不是很理想
* google api调用是自己撸了个[express api google-cloud-api-server](https://github.com/hlxxzt/google-cloud-api-server)，部署在hk服务器上用的，也有其他方案，比如：Cloudflare Workers、走镜像站、改hosts
